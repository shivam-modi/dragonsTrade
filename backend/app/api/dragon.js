   ...
   DragonTable.getDragon({dragonId})
       .then(dragon => {
         ...
         .then(({account, authenticated}) => {
            ...
            return AccountDragonTable.getDragonAccount({ dargonId });
      })
         .then(({accountId}) => {
            if(accountId == buyerId){
               throw new Error("Can't buy your own dragon!");
            }
...
router.post('/mate', (req, res, next) => {
   ...
   if(matronDragonId === patronDragonId){
      throw new Error('Cannot breed with same dragons!');
   }
   let matronDragon, 
       patronDragon,
       patronSireValue,
   ...
   getDragonWithTraits({dragonId: patronDragonId})
     ...
     .then(({account, authenticated}) => {
        ...
        if(patronSireValue > account.balance) {
           throw new Error('Sire value exceeds current account balance');
        }
